...
Public Class Class_ProyectoSiembraAcuicola
    ...
    '    Public Sub New()
    '        Me._Nombre_Catalogo = "Class_ProyectoSiembraAcuicola"
    '        Me._Nombre_Reporte = ""
    '        Me._Conexion = New SqlConnection
    '        Me._Conexion.ConnectionString = Empresa_Sistema.conexion
    '        Me._QuerySelect = "SELECT * FROM vw_cat_articulos_extendido " &
    '                            " WHERE "
    '        Me._QueryOrder = " Order by Descripcion"
    '    End Sub

    '    Public Sub New(ByVal sID_PROYECTO_SIEMBRA As String)
    '        Me.New()
    '        Try
    '            Me.ID_PROYECTO_SIEMBRA = sID_PROYECTO_SIEMBRA
    ...
    '            sqlParametro = .Parameters.Add("@ID_PROYECTO_SIEMBRA", SqlDbType.NVarChar, 16) : sqlParametro.Value = Me._ID_PROYECTO_SIEMBRA
    ...
    '                Me._Conexion.Open()
    '                .ExecuteNonQuery()
    '                Me._ID_PROYECTO_SIEMBRA = "" & .Parameters("@ID_PROYECTO_SIEMBRA").Value.ToString
    '                bResultado = True
    '            Catch ex As Exception
    ...
    '        Dim bResultado As Boolean = False
    '        Dim cmd As New SqlCommand(Me._QuerySelect & " ID_PROYECTO_SIEMBRA='" & Replace(Me._ID_PROYECTO_SIEMBRA, "'", "''") & "'", Me._Conexion)
    ...
    '                    Me._ID_PROYECTO_SIEMBRA = "" & dReader("ID_PROYECTO_SIEMBRA").ToString()
    ...
    '        Dim dsCatArticulos As New SqlDataAdapter("SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE CODIGO_CULTIVO IS NULL ORDER BY DESCRIPCION", Me._Conexion)
    '        Try
    ...
    '        Dim dTable As New DataTable
    '        Dim dsCatArticulos As New SqlDataAdapter("SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE PROTEGIDO='0' AND CODIGO_CULTIVO IS NULL ORDER BY DESCRIPCION ", Me._Conexion)
    '        Try
    '            dsCatArticulos.Fill(dTable)
    '        Catch ex As Exception
    ...
    '        Dim dTable As New DataTable
    '        Dim dsCatArticulos As New SqlDataAdapter("SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION FROM CAT_ARTICULOS A INNER JOIN CAT_CULTIVOS C ON(A.CODIGO_CULTIVO=C.CODIGO_CULTIVO) WHERE PROTEGIDO='0' AND A.CODIGO_CULTIVO IS NOT NULL AND C.CODIGO_PLAZA=" & Usuario.Codigo_Plaza & " ORDER BY DESCRIPCION  ", Me._Conexion)
    '        Try
    '            dsCatArticulos.Fill(dTable)
    '        Catch ex As Exception
    ...
    '        Try
    '            Dim sql As New Class_find("SELECT CODIGO_FAMILIA FROM CAT_ARTICULOS WHERE ID_PROYECTO_SIEMBRA ='" & sCodigoArticulo & "' ")
    '            If sql.Result1 <> "" Then
    ...
    '        Dim dTable As New DataTable
    '        Dim dA As New SqlDataAdapter("SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE PROTEGIDO='0' AND CODIGO_CULTIVO IS NULL AND DESCRIPCION LIKE '" & Filtro.ToString & "%' AND ESTATUS='" & Estatus & "' ORDER BY DESCRIPCION", Me._Conexion)
    '        Try
    '            dA.Fill(dTable)
    '        Catch ex As Exception
    ...
    '        Dim dTable As New DataTable
    '        Dim dA As New SqlDataAdapter("SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE PROTEGIDO='0' AND CODIGO_CULTIVO IS NULL AND ID_PROYECTO_SIEMBRA LIKE '" & Filtro.ToString & "%' AND ESTATUS='" & Estatus & "' ORDER BY DESCRIPCION", Me._Conexion)
    '        Try
    '            dA.Fill(dTable)
    '        Catch ex As Exception
    '            HandleError(Me._Nombre_Catalogo, "ObtenerElementosFiltro", ex)
    ...
    '        f.sCampo = "A.ID_PROYECTO_SIEMBRA"
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA FROM CAT_ARTICULOS A " &
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA,CONVERT(VARCHAR(50), CAST( ISNULL(I.EXISTENCIA,0) AS MONEY ),1) AS EXISTENCIA,CASE WHEN A.ES_SERIALIZABLE='1' THEN 'ES SERIADO' ELSE '' END " &
    '        "FROM CAT_ARTICULOS A " &
    '        "INNER JOIN CAT_FAMILIAS F ON(A.CODIGO_FAMILIA=F.CODIGO_FAMILIA) " &
    '        "LEFT JOIN CAT_CULTIVOS C ON (A.CODIGO_CULTIVO=C.CODIGO_CULTIVO) " &
    '        "LEFT JOIN INVENTARIO_EXISTENCIA_ARTICULOS I ON(I.ID_PROYECTO_SIEMBRA=A.ID_PROYECTO_SIEMBRA AND I.CODIGO_ALMACEN='" & sAlmacen & "') " &
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA,CASE WHEN A.ES_SERIALIZABLE='1' THEN 'ES SERIADO' ELSE '' END " &
    '        "FROM CAT_ARTICULOS A " &
    '        "INNER JOIN CAT_FAMILIAS F ON(A.CODIGO_FAMILIA=F.CODIGO_FAMILIA) " &
    '        "LEFT JOIN CAT_CULTIVOS C ON (A.CODIGO_CULTIVO=C.CODIGO_CULTIVO) " &
    '        "WHERE A.PROTEGIDO=0 AND A.ESTATUS='A' AND (C.CODIGO_PLAZA=" & Usuario.Codigo_Plaza.ToString & " OR A.CODIGO_CULTIVO IS NULL) AND "
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA,CONVERT(VARCHAR(50), CAST( ISNULL(I.EXISTENCIA,0) AS MONEY ),1) AS EXISTENCIA,CASE WHEN A.ES_SERIALIZABLE='1' THEN 'ES SERIADO' ELSE '' END SERIADO, " &
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA,CASE WHEN A.ES_SERIALIZABLE='1' THEN 'ES SERIADO' ELSE '' END SERIADO " &
    '        "FROM CAT_ARTICULOS A " &
    '        "INNER JOIN CAT_FAMILIAS F ON(A.CODIGO_FAMILIA=F.CODIGO_FAMILIA) " &
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA " &
    '        "FROM CAT_ARTICULOS A " &
    '        "INNER JOIN CAT_FAMILIAS F ON(A.CODIGO_FAMILIA=F.CODIGO_FAMILIA) " &
    ...
    '        f.sOrder = "A.DESCRIPCION"
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT A.ID_PROYECTO_SIEMBRA,A.DESCRIPCION,F.NOMBRE_FAMILIA,E.EXISTENCIA " &
    '        "FROM CAT_ARTICULOS A " &
    '        "INNER JOIN CAT_FAMILIAS F ON(A.CODIGO_FAMILIA=F.CODIGO_FAMILIA) " &
    '        "LEFT JOIN INVENTARIO_EXISTENCIA_ARTICULOS E ON(A.ID_PROYECTO_SIEMBRA=E.ID_PROYECTO_SIEMBRA AND E.CODIGO_ALMACEN='" & sReplace(sCodigoAlmacen) & "') " &
    '        "WHERE A.PROTEGIDO=0 AND A.ESTATUS='A' AND A.INVENTARIABLE='1' AND "
    '        f.arrayWidthColumns = New Integer() {150, 500, 250}
    '        f.Inicia("")
    '        f.ShowDialog()
    ...
    '        f.sTable = "CAT_ARTICULOS"
    '        f.sQl = "SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE PROTEGIDO=0 AND INVENTARIABLE='1' AND CODIGO_FAMILIA='0008' AND "
    ...
    '        Dim Resultado As String = ""
    '        Try
    '            Dim sql As New Class_find("SELECT DESCRIPCION FROM CAT_ARTICULOS WHERE ID_PROYECTO_SIEMBRA='" & sCodigoArticulo & "' ")
    '            If sql.Result1 <> "" Then
    '                Resultado = sql.Result1
    '            End If
    '            sql = Nothing
    ...
    '        '       "SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION FROM CAT_ARTICULOS WHERE PROTEGIDO='0' AND CODIGO_CULTIVO IS NOT NULL  ORDER BY DESCRIPCION ", Me._Conexion)
    '        f.sQl = "SELECT ID_PROYECTO_SIEMBRA,DESCRIPCION,PESO,PRECIO,CANTIDAD_BULTOS_POR_PALET FROM VW_CAT_PRODUCTOS_AGRICOLAS WHERE ESTATUS='A' AND CODIGO_PLAZA=" & Usuario.Codigo_Plaza.ToString & "AND "
    ...
    '        Dim sql As New Class_find("SELECT MAX(CAST(ID_PROYECTO_SIEMBRA AS INT)) FROM CAT_ARTICULOS WHERE ISNUMERIC(ID_PROYECTO_SIEMBRA)=1 AND ID_PROYECTO_SIEMBRA<>'-'")

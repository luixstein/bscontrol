...
#Region "Métodos y procedimientos"
    ...
    Public Function ObtenerDetalleSoloDisponibles() As DataTable
        ...
        Try
            ...
            sSQL = "SELECT CODIGO_ARTICULO, " & _
            "CASE WHEN A.ES_SERIALIZABLE = '1' THEN 'SER' WHEN A.INVENTARIABLE= '1' THEN 'INV' ELSE 'NIV' END TIPO_CONTROL_INVENTARIO, " & _
            "DISPONIBLE,PRECIO,UNIDAD_VENTA,CANTIDAD_KILOS,PRECIO_KILOS,IMPUESTO_PORCENTAJE,IMPORTE,IMPORTE_KILOS,C.CUENTA_CONTABLE,IMPUESTO_IMPORTE,ID_VENTA_DETALLE,ES_PRODUCTO_KILOS, C.CODIGO_CENTRO_COSTO " &
            "FROM VENTA_DETALLE C " & _
            "INNER JOIN CAT_ARTICULOS A ON(R.CODIGO_ARTICULO=A.CODIGO_ARTICULO) " & _
            "INNER JOIN NOMINA_CAT_CENTROS_COSTOS A ON(C.CODIGO_CENTRO_COSTO = A.CODIGO_CENTRO_COSTO)" &
            "WHERE FOLIO_VENTA='" & Me._FOLIO_VENTA & "' AND DISPONIBLE>0 " &
            "ORDER BY ID_VENTA_DETALLE "

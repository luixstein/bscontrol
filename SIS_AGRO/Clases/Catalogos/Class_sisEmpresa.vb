...
#Region "Propiedades Campos de la tabla"
    ...
    Public ReadOnly Property VALIDAR_CANCELACION_VENTAS() As Boolean
        Get
            Return Me._VALIDAR_CANCELACION_VENTAS
        End Get
    End Property
    ...
    Public ReadOnly Property TIMBRA_CFDI_HORA_ACTUAL As Boolean
        Get
    ...
    Public ReadOnly Property VALIDAR_LIMITE_CREDITO_PROVEEDORES As Boolean
        Get
            Return Me._VALIDAR_LIMITE_CREDITO_PROVEEDORES
        End Get
    End Property
    ...
    Public ReadOnly Property PRECIOS_VENTA_POR_ALMACEN As Boolean
        Get
            Return Me._PRECIOS_VENTA_POR_ALMACEN
        End Get
    End Property
...
#Region "Métodos y procedimientos"
    ...
    Public Function ObtenDocumentosSinTimbrar() As DataTable
        ...
        "SELECT FOLIO_VENTA FOLIO,'FACTURA' TIPO,FECHA,'NO ESTA TIMBRADO' PROBLEMA,case when (72-DATEDIFF(HOUR,FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM VENTA_GLOBAL " &
        "WHERE ES_FACTURA_ELECTRONICA='1' AND TIMBRADO_DESCARTADO='0' AND FECHA>'2013-24-12' AND ESTATUS_VENTA='A' AND FOLIO_FISCAL_SAT IS NULL " &
        "UNION ALL " &
        "SELECT FOLIO_DESCUENTO FOLIO,'NOTA DE CREDITO',FECHA_SERVIDOR,'NO ESTA TIMBRADO' PROBLEMA,case when (72-DATEDIFF(HOUR,FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CXC_DESCUENTOS_GLOBAL " &
        "WHERE ES_COMPROBANTE_ELECTRONICO='1' AND TIMBRADO_DESCARTADO='0' AND FECHA>'2013-24-12' AND ESTATUS_DESCUENTO='A' AND FOLIO_FISCAL_SAT IS NULL " &
        "UNION ALL " &
        "SELECT FOLIO_DEVOLUCION FOLIO,'DEVOLUCION',FECHA,'NO ESTA TIMBRADO' PROBLEMA,case when (72-DATEDIFF(HOUR,FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CXC_DEVOLUCION_GLOBAL " &
        "WHERE ES_COMPROBANTE_ELECTRONICO ='1' AND TIMBRADO_DESCARTADO='0' AND ESTATUS_DEVOLUCION='A' AND FOLIO_FISCAL_SAT IS NULL " &
        "UNION ALL " &
        "SELECT B.FOLIO_BANCO +' ' + P.FOLIO_PAGO FOLIO,'PAGO',B.FECHA_EMISION_CFDI,'NO ESTA TIMBRADO' PROBLEMA,case when (72-DATEDIFF(HOUR,B.FECHA_EMISION_CFDI, GETDATE()))>0 then 72-DATEDIFF(HOUR,B.FECHA_EMISION_CFDI, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CFDI_PAGOS_CXC_GLOBAL P " &
        "INNER JOIN BANCOS_GLOBAL B ON(P.FOLIO_BANCO=B.FOLIO_BANCO) " &
        "WHERE P.TIMBRADO_DESCARTADO ='0' AND P.ESTATUS_PAGO='A' AND P.FOLIO_FISCAL_SAT IS NULL " &
        "ORDER BY FECHA", Me._Conexion)
    ...
    Public Function ObtenDocumentosCanceladosSinTimbrar() As DataTable
        ...
        Dim da As New SqlDataAdapter(
        "SELECT V.FOLIO_VENTA FOLIO,'FACTURA' TIPO,V.FECHA,'TIMBRE SIN CANCELAR' PROBLEMA,case when (72-DATEDIFF(HOUR,V.FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,V.FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM VENTA_GLOBAL V " &
        "WHERE V.ES_FACTURA_ELECTRONICA='1' AND V.TIMBRADO_DESCARTADO='0' AND V.ESTATUS_VENTA='C' AND LEN(V.FOLIO_FISCAL_SAT)>0 AND V.ESTATUS_CANCELACION_CFDI='0' " &
        "UNION ALL " &
        "SELECT C.FOLIO_DESCUENTO FOLIO,'NOTA DE CREDITO' TIPO,C.FECHA,'TIMBRE SIN CANCELAR' PROBLEMA,case when (72-DATEDIFF(HOUR,C.FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,C.FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CXC_DESCUENTOS_GLOBAL C " &
        "WHERE C.ES_COMPROBANTE_ELECTRONICO='1' AND C.TIMBRADO_DESCARTADO='0' AND C.ESTATUS_DESCUENTO='C' AND LEN(C.FOLIO_FISCAL_SAT)>0 AND C.ESTATUS_CANCELACION_CFDI='0' " &
        "UNION ALL " &
        "SELECT C.FOLIO_DEVOLUCION FOLIO,'DEVOLUCION' TIPO,C.FECHA,'TIMBRE SIN CANCELAR' PROBLEMA,case when (72-DATEDIFF(HOUR,C.FECHA, GETDATE()))>0 then 72-DATEDIFF(HOUR,C.FECHA, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CXC_DEVOLUCION_GLOBAL C " &
        "WHERE C.ES_COMPROBANTE_ELECTRONICO='1' AND C.TIMBRADO_DESCARTADO='0' AND C.ESTATUS_DEVOLUCION='C' AND LEN(C.FOLIO_FISCAL_SAT)>0 AND C.ESTATUS_CANCELACION_CFDI='0' " &
        "UNION ALL " &
        "SELECT B.FOLIO_BANCO+' ' + P.FOLIO_PAGO FOLIO,'PAGO' TIPO,B.FECHA_EMISION_CFDI,'TIMBRE SIN CANCELAR' PROBLEMA,case when (72-DATEDIFF(HOUR,B.FECHA_EMISION_CFDI, GETDATE()))>0 then 72-DATEDIFF(HOUR,B.FECHA_EMISION_CFDI, GETDATE()) else 0 end TIEMPO_RESTANTE " &
        "FROM CFDI_PAGOS_CXC_GLOBAL P " &
        "INNER JOIN BANCOS_GLOBAL B ON(P.FOLIO_BANCO=B.FOLIO_BANCO) " &
        "WHERE P.TIMBRADO_DESCARTADO='0' AND P.ESTATUS_PAGO='C' AND LEN(P.FOLIO_FISCAL_SAT)>0 AND P.ESTATUS_CANCELACION_CFDI='0' " &
        "ORDER BY FECHA", Me._Conexion)

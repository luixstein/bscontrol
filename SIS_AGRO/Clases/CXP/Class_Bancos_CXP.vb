...
#Region "Métodos y procedimientos"
    ...
    Public Function ObtenerDetalle() As DataTable
        ...
        sSQL = "SELECT B.FOLIO_REFERENCIA_USUARIO,ISNULL(Convert(varchar(10),B.FECHA, 103),'') FECHA,B.FOLIO_REFERENCIA,B.NOMBRE_MONEDA_CO,B.TIPO_DE_CAMBIO_CO,B.SALDO_USD_CO,B.CONCEPTO_COMPRA," & _
               "B.TOTAL_COMPRA,B.SALDO_COMPRA,0 SALDO_IMPUESTO,B.RETENCION_IVA,0 TOTAL_DETALLE_IMPUESTO,B.TOTAL_DETALLE,B.TOTAL_DETALLE_DOLARES,0,B.CODIGO_DOCUMENTO_COMPRA,'' " & _
               "FROM VW_BANCOS_CXP_DETALLE B " & _
               "WHERE FOLIO_BANCO='" & Me._FOLIO_BANCO & "' AND CODIGO_PLAZA= " & Plaza.CODIGO_PLAZA & " " & _
               "ORDER BY FECHA"
        Try
    ...
    Public Function CargaComprasProveedorConSaldo(ByVal CodigoProveedor As String) As DataTable
        ...
        Dim sSQL As String = ("SELECT FOLIO_PROVEEDOR,ISNULL(Convert(varchar(10),G.FECHA, 103),'') FECHA,G.FOLIO_COMPRA,M.ABREVIACION NOMBRE_MONEDA_CO,G.TIPO_DE_CAMBIO,G.SALDO_DOLARES,LEFT(CONCEPTO,40) CONCEPTO, " & _
                              ...
                              "LEFT JOIN CXP_PAGOS_AUTORIZADOS A ON(G.FOLIO_COMPRA=A.FOLIO_COMPRA AND A.ESTATUS_AUTORIZACION_USADA='0') " & _
                              "INNER JOIN CATALOGO_MONEDAS M ON(G.CODIGO_MONEDA=M.CODIGO_MONEDA)" & _

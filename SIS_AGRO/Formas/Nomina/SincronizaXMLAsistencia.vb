...
Public Class SincronizaXMLAsistencia

    Private dtAsistencia As New DataTable("dtAsistencia")
    ...
    Private oListado As New Class_NominaListado
...
#Region "Propiedades"
    Private ReadOnly Property NombreClase() As String
        Get
            NombreClase = "SincronizaXMLAsistencia"
        End Get
    End Property
#End Region
...
#Region "Métodos y procedimientos"
    ...
    Private Function sincronizaNominaListado() As Boolean
        Try
            If Me.CargaXMLAsistencia = False Then
                MsgBox("No se cargó la información de la asistencia, no es posible sincronizar.", MsgBoxStyle.Exclamation, Me.Text)
                Me.btnSincronizaAgrinet.Enabled = True
                Exit Function
            End If

            If Me.grabaLista() = True Then
                MsgBox("Listado grabado correctamente", MsgBoxStyle.Information)
            Else
                MsgBox("Error al grabar el listado", MsgBoxStyle.Critical)
            End If
        Catch ex As Exception
            HandleError(Me.NombreClase, "sincronizaListado", ex)
        End Try
    ...
    Public Function CargaXMLAsistencia() As Boolean
        Dim dInicio As Date = Date.Now
        Try
            Dim XmlAsistenciaArchivoZIP As String = Me.txtRutaArchivo.Text
            Dim XmlAsistenciaArchivoXML As String = IIf(XmlAsistenciaArchivoZIP.Length > 0, Path.ChangeExtension(Me.txtRutaArchivo.Text, "xml"), "").ToString
            Dim XmlAsistenciaSchema As String = IIf(XmlAsistenciaArchivoZIP.Length > 0, Path.GetDirectoryName(XmlAsistenciaArchivoZIP) & "\Asistencias.xsl", "").ToString

            If txtLEN(XmlAsistenciaArchivoZIP) = False Then
                MsgBox("No seleccionó ningún archivo.", MsgBoxStyle.Exclamation, Me.Text)
                Exit Function
            End If

            If File.Exists(XmlAsistenciaArchivoZIP) = False Then
                MsgBox("El archivo xml seleccionado no existe. Verifíquelo.", MsgBoxStyle.Exclamation, Me.Text)
                Exit Function
            End If

            If Me.Unzip(XmlAsistenciaArchivoZIP) = False Then
                Exit Function
            End If

            If File.Exists(XmlAsistenciaSchema) = False Then 'Se pregunta despues porque va dentro del zip.
                MsgBox("El archivo xms(schema) no existe. Avíse al depto. de sistemas.", MsgBoxStyle.Exclamation, Me.Text)
                Exit Function
            End If

            Me.dtAsistencia.Dispose()
            Me.dtAsistencia = New DataTable("dtAsistencia")

            Dim fsReadXml As New System.IO.FileStream(XmlAsistenciaArchivoXML, System.IO.FileMode.Open)
            Dim fsReadXmlSchema As New System.IO.FileStream(XmlAsistenciaSchema, System.IO.FileMode.Open)
            ...
            Me.dtAsistencia.ReadXmlSchema(myXmlReaderSchema)
            Me.dtAsistencia.ReadXml(myXmlReader)

            myXmlReaderSchema.Close()
            myXmlReader.Close()

            CargaXMLAsistencia = True
        Catch ex As Exception
            HandleError(Me.NombreClase, "CargaXMLAsistencia", ex)
        End Try
    End Function

    Private Function grabaLista() As Boolean
        Dim bResultado As Boolean = False
        Me.ProgressBar1.Maximum = dtAsistencia.Rows.Count
        Try
            For Each dRow As DataRow In dtAsistencia.Rows
                With oListado
                    .CODIGO_TRABAJADOR = dRow("CODIGO_TRABAJADOR").ToString
                    .TURNO = dRow("TURNO").ToString
                    .TIPO = dRow("TIPO").ToString
                    .CODIGO_VEHICULO = dRow("CODIGO_VEHICULO").ToString
                    .FECHA = CDate(dRow("FECHA"))
        ...
        Catch ex As Exception
            HandleError(Me.NombreClase, "grabaLista", ex)
        End Try
    ...
    '    Try
    '        Dim HJS = Me.dtAsistencia.AsEnumerable()
